package ch.unibas.dmi.dbis.polyphenydb.client.scenarios.tpch;


import static ch.unibas.dmi.dbis.polyphenydb.client.rpc.ProtoObjectFactory.StopWorkerMessage;
import static ch.unibas.dmi.dbis.polyphenydb.client.rpc.ProtoObjectFactory.TPCHWorkerMessage;
import static ch.unibas.dmi.dbis.polyphenydb.client.rpc.ProtoObjectFactory.createFetchTPCHMessage;

import ch.unibas.dmi.dbis.polyphenydb.client.analysis.tpch.TPCHAnalysis;
...
import ch.unibas.dmi.dbis.polyphenydb.client.db.tpch.IcarusTpchBenchmarker;
import ch.unibas.dmi.dbis.polyphenydb.client.db.tpch.PostgresTpchBenchmarker;
...
public class TPCHScenario implements Scenario {
    ...
    private void fetchResults( long start, long stop ) {
        ...
        for ( WorkerStub workerStub : workers ) {
            ...
            workerStub.writeTPCHResults( this.resultWriter, createFetchTPCHMessage( start, stop ) );
        }
    ...
    private void launchWorkers() {
        ...
        for ( int i = 0; i < workerURLs.length; i++ ) {
            ...
            LaunchWorkerMessage workerMessage = TPCHWorkerMessage( job.getEvaluation().getDbms().getHost(), job.getEvaluation().getDbms().getPort(), job.getEvaluation().getDbms().getDatabase(), job.getEvaluation().getDbms().getUsername(), job.getEvaluation().getDbms().getPassword(),
    ...
    public void abort() {
        ...
        for ( WorkerStub worker : workers ) {
            worker.stopWorker( StopWorkerMessage( PolyClientGRPC.Scenario.SCENARIOTPCH ) );
        }
    }
