...
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.TPCHPopulationGenerators.OrderGenerator.createOrderDateRandom;
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.TPCHPopulationGenerators.OrderGenerator.makeOrderKey;
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.TPCHPopulationGenerators.PartSupplierGenerator.selectPartSupplier;
...
public class LineItemGenerator
    ...
    private static class LineItemGeneratorIterator
        ...
        private final RandomBoundedInt supplierNumberRandom = new RandomBoundedInt( 2095021727, 0, 3,
                LINE_COUNT_MAX );

        private final RandomBoundedInt shipDateRandom = createShipDateRandom();
        private final RandomBoundedInt commitDateRandom = new RandomBoundedInt( 904914315,
                COMMIT_DATE_MIN, COMMIT_DATE_MAX, LINE_COUNT_MAX );
        private final RandomBoundedInt receiptDateRandom = new RandomBoundedInt( 373135028,
                RECEIPT_DATE_MIN, RECEIPT_DATE_MAX, LINE_COUNT_MAX );
        ...
        private LineItemGeneratorIterator( Distributions distributions, TextPool textPool,
                double scaleFactor, long startIndex, long rowCount ) {
            this.scaleFactor = scaleFactor;
            this.startIndex = startIndex;
            this.rowCount = rowCount;

            returnedFlagRandom = new RandomString( 717419739, distributions.getReturnFlags(),
                    LINE_COUNT_MAX );
            shipInstructionsRandom = new RandomString( 1371272478, distributions.getShipInstructions(),
                    LINE_COUNT_MAX );
            shipModeRandom = new RandomString( 675466456, distributions.getShipModes(), LINE_COUNT_MAX );
            commentRandom = new RandomText( 1095462486, textPool, COMMENT_AVERAGE_LENGTH,
                    LINE_COUNT_MAX );
