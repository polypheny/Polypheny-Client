...
import static ch.unibas.dmi.dbis.polyphenydb.client.rpc.ProtoObjectFactory.MusqleResultTuple;
...
public abstract class MusqleBenchmarker {
    ...
    public MUSQLEResultTuple genericQueryExecutor( int queryID, DBConnector connector, String query ) {
        ...
        try ( ResultSet resultSet = connector.executeQuery( query ) ) {
            ...
            return MusqleResultTuple( start, (System.currentTimeMillis() - start), transactionType, queryID, true, query );
            //abort();
            //throw new RuntimeException( e );
        }
        logger.trace( "Finished Q{} with text {}", queryID, query );
        return MusqleResultTuple( start, (stop - start), transactionType, queryID, false, query );
    }
