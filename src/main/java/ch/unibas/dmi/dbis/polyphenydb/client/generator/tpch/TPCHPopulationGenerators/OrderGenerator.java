...
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.TPCHPopulationGenerators.LineItemGenerator.ITEM_SHIP_DAYS;
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.TPCHPopulationGenerators.LineItemGenerator.createDiscountRandom;
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.TPCHPopulationGenerators.LineItemGenerator.createPartKeyRandom;
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.TPCHPopulationGenerators.LineItemGenerator.createQuantityRandom;
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.TPCHPopulationGenerators.LineItemGenerator.createShipDateRandom;
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.TPCHPopulationGenerators.LineItemGenerator.createTaxRandom;
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.TPCHPopulationGenerators.PartGenerator.calculatePartPrice;
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.objects.GenerateUtils.MIN_GENERATE_DATE;
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.objects.GenerateUtils.TOTAL_DATE_RANGE;
...
import static ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.objects.GenerateUtils.toEpochDate;
...
public class OrderGenerator
    ...
    public OrderGenerator( double scaleFactor, int part, int partCount, Distributions distributions,
            TextPool textPool ) {
    ...
    private static class OrderGeneratorIterator
        ...
        private OrderGeneratorIterator( Distributions distributions, TextPool textPool,
                double scaleFactor, long startIndex, long rowCount ) {
            this.startIndex = startIndex;
            this.rowCount = rowCount;

            clerkRandom = new RandomBoundedInt( 1171034773, 1,
                    Math.max( (int) (scaleFactor * CLERK_SCALE_BASE), CLERK_SCALE_BASE ) );

            maxCustomerKey = (long) (CustomerGenerator.SCALE_BASE * scaleFactor);
            customerKeyRandom = new RandomBoundedLong( 851767375, scaleFactor >= 30000, 1,
                    maxCustomerKey );
        ...
        private Order makeOrder( long index ) {
            ...
            for ( long lineNumber = 0; lineNumber < lineCount; lineNumber++ ) {
                ...
                shipDate += orderDate;
                if ( ch.unibas.dmi.dbis.polyphenydb.client.generator.tpch.objects.GenerateUtils
                        .isInPast( shipDate ) ) {
                    shippedCount++;
                }
            }
